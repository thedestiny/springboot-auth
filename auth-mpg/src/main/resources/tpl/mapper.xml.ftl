...
<mapper namespace="${package.Mapper}.${table.mapperName}">
    ...
    <insert id="updateByEntityId" parameterType="${package.Entity}.${table.entityName}">
        ...
        <set>
            <#list table.fields as field>
                <#if !field.keyFlag><#--生成主键排在第一位-->
            <if test="${field.propertyName} != null<#if field.propertyType == "String"> and ${field.propertyName} != ''</#if>">${field.name} = ${r'#{'}${field.propertyName}${r"}"}<#if field_has_next>,</#if></if>
                </#if>
            </#list>
        </set>
        <#list table.fields as field>
    ...
    <select id="queryEntityList" resultMap="BaseResultMap">
        ...
        <where>
            <#list table.fields as field>
                <#if !field.keyFlag><#--生成主键排在第一位-->
            <if test="${field.propertyName} != null">and ${field.name} = ${r'#{'}${field.propertyName}${r"}"}<#if field_has_next>,</#if></if>
                </#if>
            </#list>
        </where>
    </select>
